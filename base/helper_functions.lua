...
function mod.add_text_zh(self, text_id, text)
    if type(text) == "table" then
        _localization_database[text_id] = text
    else
        _localization_database[text_id] = {
...
function mod.update_buff_templates()
...
function mod.add_buff_template(self, buff_name, buff_data)   
    local new_talent_buff = {
        buffs = {
            merge({ name = buff_name }, table.clone(buff_data)),
        },
    }
    BuffTemplates[buff_name] = new_talent_buff
    local index = #NetworkLookup.buff_templates + 1
...
mod.add_proc_event = function (proc_event_name)
    local index = #ProcEvents+1
    ProcEvents[index] = proc_event_name
    ProcEvents[proc_event_name] = index
    local index = #NetworkLookup.proc_events + 1
    NetworkLookup.proc_events[index] = proc_event_name
    NetworkLookup.proc_events[proc_event_name] = index
end
...
--[[
function mod.add_action_class(self, class_name, class)
    settings.action_classes_lookup = {
        settings.action_classes_lookup[class_name] = class
end
--]]
...
mod.auto_enable_new_weapons = function ()
...
local lingering_tickrate_reduction = 1

mod.update_weapons =  function ()
    mod.generate_infinite_burn_variants(BuffTemplates)
